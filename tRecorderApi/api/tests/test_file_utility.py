"""
    This module contains the suite of tests for testing the FileUtility class
    in the api.file_transfer module.
"""
...
class FileUtilityTest(TestCase):
    """
        Test cases for testing the behaviors of the individual methods of the
        FileUtility class.
    """
    def mock_progress(self, task):
        """Mocked version of the update_progress function"""
        return task

    def join_return(self, directory, file_name):
        """Mock function to replace os.path.join"""
    ...
    def test_get_markers(
        ...
        m_tiny_tag):
        """
            Testing the behaviors of the get_markers() function. This function
            expects an object of type TinyTag as defined in the TinyTag class
            inside the api.file_transfer.tinytag module. TinyTag.get takes in a
            wav file as input. The expected output of TinyTag.get() is
            a JSON object containging the info from the markers key inside the
            take_info object of the manifest json.
        """
        # Build dependencies
        m_tiny_tag.artist = "Fake Person"


    @patch('api.file_transfer.os.path.join')
    @patch('builtins.open', new_callable=mock_open())
    @patch('json.load')
    def test_open_manifest_file(
            self,
            m_json,
            m_open,
            mock_file_path):
        """
            Verify that the open_manifest_file() function opens a JSON file and
            loads in that JSON file.
        """
        mock_file_path.side_effect = self.join_return
        self.file_util.open_manifest_file("fake_path/")
        m_open.assert_called_with("fake_path/manifest.json")
        m_json.assert_called_with(m_open.return_value.__enter__.return_value)
