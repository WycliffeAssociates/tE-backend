...
class TestAnthologyView(TestCase):
    ...
    def test_get_anthology_without_slug_returns_success(self):
        response = self.client.get(base_url + 'get_anthologies/',format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_get_anthology_with_slug_returns_success(self):
        response = self.client.post(base_url + 'get_anthologies/',{"slug":"old"},format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_get_anthology_with_slug_returns_204(self):
        response = self.client.post(base_url + 'get_anthologies/',{"slug":"oldd"},format='json')
